 ...
 new ol.layer.Tile(
        ...
        baseLayer: false,
          source: action_plan = new ol.source.TileWMS({
            url: base_url + '/wms',
            params: {'LAYERS': 'APWS:namsai_land_and_water_action_plan', 'TILED': true},
           // format : new ol.format.GeoJSON,
            serverType: 'geoserver',
            // Countries have transparency, so do not fade tiles:
            transition: 0
...
map.on('singleclick', function (evt) {
  ...
  var url = grid_layer.getGetFeatureInfoUrl(
...
function getAPReport() {
  ...
  postAPData("../namsai1/apreport.php", data, "POST")
}
...
function getVisibleLayerslist() {
  ...
  all_layers.forEach(layer => {
    ...
    if ((layervisability == true)  && (layer.getProperties().name == 'namsai_land_and_water_action_plan'))  {
